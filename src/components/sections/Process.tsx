    ...
    <section className="section" aria-label="Unser Prozess in 4 Schritten">
      <div className="container">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-bold">In 4 Schritten zu messbarem Wachstum</h2>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {processData.map((step, i) => (
              <div key={i} className="relative text-center group">
                {/* Step Number Circle */}
                <div className="relative z-10 w-16 h-16 mx-auto mb-6 rounded-2xl bg-[rgba(27,152,224,0.08)] border border-[rgba(27,152,224,0.2)] flex items-center justify-center group-hover:bg-[rgba(27,152,224,0.15)] group-hover:border-[rgba(27,152,224,0.4)] transition-all duration-300">
                  <img src={step.image} alt={`Schritt ${i + 1}: ${step.heading}`} className="w-8 h-8" />
                </div>

                {/* Step Label */}
                <span className="text-xs font-semibold text-[var(--color-accent)] tracking-wider uppercase mb-2 block">
                  Schritt {i + 1}
                </span>

                <h3 className="text-lg font-semibold mb-3">{step.heading}</h3>
                <p className="text-sm text-[rgba(255,255,255,0.5)] leading-relaxed">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
