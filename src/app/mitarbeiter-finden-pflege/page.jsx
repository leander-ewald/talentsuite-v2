  ...
  const submit = async () => {
    ...
    try {
      await fetch("/api/leadmagnet-capture", {
        ...
        body: JSON.stringify({
          source: "mitarbeiter-finden-pflege", name: form.name, email: form.email,
          phone: form.phone || undefined, company: form.company || undefined,
          industry: "Pflege & Gesundheit",
          extra: { branche: "Pflege & Gesundheit", pageType: "branchen-landing" },
        }),
      });
      ...
      <div style={{ background: `linear-gradient(135deg, ${D} 0%, ${B} 50%, #0A4D72 100%)`, padding: mob ? "48px 16px 56px" : "80px 24px 88px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center", position: "relative" }}>
          ...
          <h1 style={{ color: W, fontSize: mob ? 26 : 42, fontWeight: 800, margin: "0 0 16px", lineHeight: 1.2 }}>
            Pflegekräfte finden — Pflegefachkräfte & Pflegehelfer in 30 Tagen
          </h1>
          <p style={{ color: `${W}80`, fontSize: mob ? 15 : 19, maxWidth: 620, margin: "0 auto 24px", lineHeight: 1.6 }}>
            46.000+ offene Stellen · Ø 200+ Tage Vakanzzeit · 72% der Fachkräfte nicht auf Jobbörsen
          </p>
          ...
          <p style={{ color: `${W}50`, fontSize: 13, marginTop: 10 }}>20 Min. · Unverbindlich · Konkrete Strategie für Pflege & Gesundheit</p>
        </div>
      </div>
      ...
      <div style={{ background: L, padding: mob ? "32px 16px 48px" : "56px 24px 72px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 32 }}>
            <Stat n="46.000+" label="Offene Stellen" />
            <Stat n="200+ Tage" label="Ø Vakanzzeit" color={R} />
            <Stat n="18-28 €" label="Kosten/Bewerbung" color={G} />
            <Stat n="Ø 35 Tage" label="Bis zur Einstellung" />
          </div>
          ...
          <div style={{ background: W, borderRadius: 16, padding: mob ? "24px 18px" : "32px 28px", marginBottom: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
            <h2 style={{ color: B, fontSize: mob ? 20 : 26, fontWeight: 700, margin: "0 0 14px" }}>
              🏥 Die Situation in der Alten- und Krankenpflege
            </h2>
            <p style={{ color: "#4A5568", fontSize: 15, lineHeight: 1.7, margin: "0 0 16px" }}>
              Die Pflege steht vor einer Belastungskrise. Steigende Patientenzahlen, alternde Belegschaften und ein dramatischer Nachwuchsmangel. Die Vakanzzeiten in der Pflege sind die höchsten aller Branchen.
            </p>
            <div style={{ padding: "14px 16px", background: i === 0 ? `${R}05` : `${L}`, border: `1px solid ${i === 0 ? R + "15" : "#E2E8F0"}`, borderRadius: 10, marginBottom: 10 }}>
              <p style={{ color: "#334155", fontSize: 15, lineHeight: 1.6, margin: 0 }}><strong>🔴</strong> Abwärtsspirale: Jede unbesetzte Stelle erhöht die Belastung des bestehenden Teams — was zu weiteren Kündigungen führt.</p>
            </div>
            <div style={{ padding: "14px 16px", background: i === 0 ? `${R}05` : `${L}`, border: `1px solid ${i === 0 ? R + "15" : "#E2E8F0"}`, borderRadius: 10, marginBottom: 10 }}>
              <p style={{ color: "#334155", fontSize: 15, lineHeight: 1.6, margin: 0 }}><strong>🟡</strong> Längste Vakanzzeiten: Mit über 200 Tagen durchschnittlicher Besetzungszeit ist die Pflege Schlusslicht beim Recruiting.</p>
            </div>
            <div style={{ padding: "14px 16px", background: i === 0 ? `${R}05` : `${L}`, border: `1px solid ${i === 0 ? R + "15" : "#E2E8F0"}`, borderRadius: 10, marginBottom: 0 }}>
              <p style={{ color: "#334155", fontSize: 15, lineHeight: 1.6, margin: 0 }}><strong>🟠</strong> Wechselbereitschaft: 72% der Pflegekräfte sind nicht aktiv auf Jobsuche, aber offen für bessere Angebote — Sie müssen sie nur erreichen.</p>
            </div>
          </div>
          ...
          <div style={{ background: `${R}05`, border: `1px solid ${R}15`, borderRadius: 16, padding: mob ? "24px 18px" : "32px 28px", marginBottom: 24 }}>
            <h2 style={{ color: B, fontSize: mob ? 18 : 22, fontWeight: 700, margin: "0 0 12px" }}>
              ❌ Warum Jobbörsen im Pflege & Gesundheit nicht mehr funktionieren
            </h2>
            <p style={{ color: "#4A5568", fontSize: 15, lineHeight: 1.7, margin: 0 }}>
              Pflegekräfte sind erschöpft vom Alltag — sie verbringen ihre freie Zeit auf Instagram und Facebook, nicht auf Jobbörsen. Wer sie dort nicht anspricht, verliert sie an die Konkurrenz.
            </p>
          </div>
          ...
          <div style={{ background: W, borderRadius: 16, padding: mob ? "24px 18px" : "32px 28px", marginBottom: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
            <h2 style={{ color: B, fontSize: mob ? 18 : 22, fontWeight: 700, margin: "0 0 14px" }}>
              ✅ Social Recruiting für Pflege & Gesundheit: So funktioniert's
            </h2>
            <p style={{ color: "#4A5568", fontSize: 15, lineHeight: 1.7, margin: "0 0 16px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr 1fr", gap: 12 }}>
              {[
                { icon: "🎯", t: "Gezielte Ansprache", d: "Wir erreichen Pflegefachkraft (m im Umkreis von 30km — nach Beruf, Alter und Interessen." },
                ...
                { icon: "📊", t: "Messbare Ergebnisse", d: "18-28 € pro Bewerbung bei 14-18% Conversion. Transparent und nachvollziehbar." },
              ].map((s, i) => (
          ...
          <div style={{ background: `${G}08`, border: `1px solid ${G}20`, borderRadius: 16, padding: mob ? "24px 18px" : "32px 28px", marginBottom: 24 }}>
            ...
            <p style={{ color: "#334155", fontSize: 15, lineHeight: 1.7, margin: 0 }}>
              Ein Pflegeheim in Niedersachsen mit 80 Betten fand 10 Monate keinen examinierten Pflegefachkraft. Social Recruiting: 31 Bewerbungen in 5 Wochen, 3 Einstellungen.
            </p>
          </div>
          ...
          <div style={{ background: W, borderRadius: 16, padding: mob ? "24px 18px" : "32px 28px", marginBottom: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
            <h2 style={{ color: B, fontSize: mob ? 18 : 22, fontWeight: 700, margin: "0 0 14px" }}>
              Diese Positionen besetzen wir im Pflege & Gesundheit
            </h2>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {["Pflegefachkraft (m/w/d)","Altenpfleger/in","Krankenpfleger/in","Pflegehelfer/in","Pflegedienstleitung"].map((p, i) => (
          ...
          <div style={{ background: W, borderRadius: 16, padding: mob ? "20px 18px" : "28px 28px", marginBottom: 24, boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
            ...
            <p style={{ color: "#4A5568", fontSize: 15, lineHeight: 1.7, margin: 0 }}>
              Die Ergebnisse sprechen für sich: 18-28 € pro qualifizierte Bewerbung, 14-18% Conversion-Rate und durchschnittlich 35 Tage bis zur Einstellung. Deutlich schneller und kostengünstiger als traditionelle Wege über Jobbörsen, Zeitungsanzeigen oder Personalvermittler.
            </p>
          </div>
