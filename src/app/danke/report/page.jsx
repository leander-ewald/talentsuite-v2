...
export default function Danke() {
  ...
  useEffect(() => { if (!tracked) { trackMetaLead({ formName: "danke-report", category: "leadmagnet-danke", value: 50 }); setTracked(true); } }, [tracked]);

  return (
    <>
      <Head><title>Danke — Branchenreport | TalentSuite</title></Head>
      <div style={{ background: "#f0f4f7", minHeight: "100vh" }}>
        <div style={{ background: "linear-gradient(135deg, #011E2F 0%, #023B5B 50%, #0A4D72 100%)", padding: mob ? "64px 16px 72px" : "96px 24px 104px", textAlign: "center" }}>
          ...
          <h1 style={{ color: "#ffffff", fontSize: mob ? 24 : 36, fontWeight: 800, margin: "0 0 12px", lineHeight: 1.15 }}>Ihr Report wird gesendet!</h1>
          <p style={{ color: "rgba(255,255,255,0.7)", fontSize: mob ? 15 : 18, maxWidth: 520, margin: "0 auto" }}>Prüfen Sie Ihr Postfach — Der Branchenreport 2026 kommt in den nächsten Minuten.</p>
        </div>
        <div style={{ maxWidth: 600, margin: "0 auto", padding: mob ? "24px 16px 48px" : "40px 24px 64px" }}>
          <div style={{ background: "#ffffff", borderRadius: 16, padding: mob ? "24px 18px" : "32px 28px", boxShadow: "0 8px 40px rgba(0,0,0,0.08)", marginTop: mob ? -32 : -48 }}>
            ...
            <p style={{ color: "#4A5568", fontSize: 15, lineHeight: 1.7, margin: "0 0 20px", textAlign: "center" }}>
              Sie haben den Branchenreport erhalten — aber möchten Sie wissen, was das konkret für Ihren Betrieb bedeutet?
            </p>
